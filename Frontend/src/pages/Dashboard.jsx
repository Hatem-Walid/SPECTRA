  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div
        ...
        style={{
          background: "radial-gradient(circle at 32% 28%, #9db4ff 0%, #5f74e8 38%, #2c3570 72%, #10122a 100%)",
          boxShadow: active ? "0 0 22px 4px rgba(108,140,255,0.45), 0 0 60px 10px rgba(108,140,255,0.15)" : "0 0 10px 2px rgba(108,140,255,0.25)",
      ...
      <div
        className="absolute inset-0 rounded-full"
        style={{
          background: "conic-gradient(from 90deg, transparent 0%, rgba(212,175,106,0.55) 18%, transparent 40%)",
    ...
    >
      ...
      {booting ? (
        ...
        <div className="w-full h-full md:h-[760px] md:max-w-[1180px] bg-[#0e111a]/90 backdrop-blur-xl md:rounded-[1.75rem] shadow-[0_30px_80px_rgba(0,0,0,0.55)] border border-white/[0.06] flex overflow-hidden relative">
          ...
          {/* زرار يرجعك للـ Landing لو حابب */}
          {onBackToHome && (
            <button 
              onClick={onBackToHome}
              className="absolute top-3 right-4 z-50 text-[10px] text-gray-400 hover:text-white bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg transition"
            >
              ← Back to Home
            </button>
          )}
            ...
            <div className="flex flex-col items-center gap-4 md:gap-5">
              ...
              <div className="flex flex-col gap-3 mt-1 text-[#5b6274]">
                ...
                <div className="relative">
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border flex items-center justify-center transition ${
                      !EXAMPLES[activeTool].length ? "opacity-30 cursor-not-allowed border-white/[0.07]" : showExamples ? "border-[#6c8cff] text-[#b9c6ff]" : "border-white/[0.07] hover:text-white/80 hover:border-white/20"
                    }`}
                  >
                    <ZapIcon size={15} />
                  </button>
                  {showExamples && EXAMPLES[activeTool].length > 0 && (
                    <div className="absolute left-11 top-0 z-20 w-60 bg-[#161a26] border border-white/[0.08] rounded-xl shadow-2xl p-1.5 fade-up">
                      {EXAMPLES[activeTool].map((ex) => (
                        <button
                          key={ex.label}
                          onClick={() => { handleSend(ex.text); setShowExamples(false); }}
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-full border flex items-center justify-center transition ${
                    activeTool === "leaf" ? "opacity-30 cursor-not-allowed border-white/[0.07]" : manualMode ? "border-[#6c8cff] text-[#b9c6ff]" : "border-white/[0.07] hover:text-white/80 hover:border-white/20"
                  }`}
                >
                  <CodeIcon size={15} />
                </button>
            ...
            {!hasStarted ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-5 fade-up">
                ...
                <div className="text-center">
                  ...
                  <h1 className="text-[26px] md:text-[30px] leading-snug text-[#eef0f7]" style={{ fontFamily: "var(--font-display)", fontWeight: 550 }}>
                ...
                {loading && (
                  <div className="flex items-center gap-2.5 pl-1 fade-up">
                    ...
                    <span className="flex gap-1">
                      {[0, 1, 2].map((i) => (
                        <span key={i} className="w-1 h-1 rounded-full bg-[#8b92a5]" style={{ animation: `dot-bounce 1.1s ${i * 0.15}s infinite ease-in-out` }} />
                      ))}
                    </span>
                  </div>
                )}
                    ...
                    {TOOLS.map((t) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 p-2 px-2.5 rounded-xl text-[11px] font-medium border transition text-left ${
                          t.id === activeTool ? "bg-[#6c8cff]/[0.12] border-[#6c8cff]/40 text-[#eef0f7]" : "bg-white/[0.02] border-white/[0.05] text-[#a6acc0] hover:bg-white/[0.05] hover:border-white/[0.1]"
                        }`}
                      >
...
function BotAvatar() {
  return (
    <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg,#2c3050,#171a29)", border: "1px solid rgba(108,140,255,0.35)" }}>
...
function MessageBubble({ message, onCopy, copied }) {
  ...
  if (message.kind === "file") {
    return (
      <div className="flex flex-col items-end fade-up">
        <div className="flex items-center gap-2 bg-white/[0.06] border border-white/[0.09] rounded-xl px-3 py-2 max-w-[220px]">
          <FileTypeIcon size={15} className="text-[#b9c6ff] shrink-0" />
          <span className="text-[11px] text-[#e4e6ef] truncate">{message.fileName}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col fade-up ${isUser ? "items-end" : "items-start"}`}>
      <div className={isUser ? "flex flex-col items-end" : "flex items-start gap-2 max-w-[340px]"}>
        {!isUser && <BotAvatar />}
        <div
          className={isUser ? "text-[#0b0d15] text-[11px] px-4 py-2.5 rounded-2xl rounded-tr-sm max-w-[280px] font-medium" : "bg-white/[0.04] border border-white/[0.06] text-[#d4d7e3] text-[11px] px-4 py-2.5 rounded-2xl rounded-tl-sm leading-relaxed"}
...
function ResultCard({ tool, data, onCopy, copied }) {
  ...
  return (
    <div className={`${shell} w-56`}>
      {copyBtn}
      <div className={`text-[12px] font-semibold mb-2 ${isHealthy ? "text-emerald-400" : "text-[#e4e6ef]"}`}>
        {mainLabel}
      </div>
      {data.top_3?.map((item) => (
